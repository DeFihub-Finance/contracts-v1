...
import { ERC20, StrategyManager__v3 } from '@src/typechain'
    ...
    describe('given a strategist with no rewards to collect', () => {
        describe('when the strategist collects the rewards', () => {
            it('then the strategist\'s balance remains unchanged', async () => {
                ...
                await strategyManager.connect(account1).collectRewards(stablecoin)
                expect(await strategistBalance()).to.equal(balanceBefore)
            })

            it('then emits a CollectedStrategistRewards event', async () => {
                const tx = strategyManager.connect(account1).collectRewards(stablecoin)

                await expect(tx).to.not.emit(strategyManager, 'CollectedStrategistRewards')
                await expect(tx).to.not.emit(strategyManager, 'CollectedRewards')
            })
        })
    })
})
