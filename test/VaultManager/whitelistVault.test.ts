    ...
    describe('given the owner of VaultManager', () => {
        describe('when setVaultWhitelistStatus is called with true', () => {
            it('then the vault is whitelisted', async () => {
                await vaultManager.connect(owner).setVaultWhitelistStatus(await vault.getAddress(), true)

                expect(await vaultManager.whitelistedVaults(await vault.getAddress())).to.equal(true)
            })

            it('then VaultWhitelisted event is emitted with the vault address and true', async () => {
                await expect(vaultManager.connect(owner).setVaultWhitelistStatus(await vault.getAddress(), true))
        ...
        describe('when setVaultWhitelistStatus is called with false', () => {
            it('then the vault is blacklisted', async () => {
                await vaultManager.connect(owner).setVaultWhitelistStatus(await vault.getAddress(), false)

                expect(await vaultManager.whitelistedVaults(await vault.getAddress())).to.equal(false)
            })

            it('then VaultWhitelisted event is emitted with the vault address and false', async () => {
                await expect(vaultManager.connect(owner).setVaultWhitelistStatus(await vault.getAddress(), false))
                    .to.emit(vaultManager, 'VaultWhitelisted')
    ...
    describe('given a non-owner of VaultManager', () => {
        describe('when setVaultWhitelistStatus is called', () => {
            it('then transaction reverts with Unauthorized', async () => {
                ...
                await expect(vaultManager.connect(account0).setVaultWhitelistStatus(await vault.getAddress(), true))
